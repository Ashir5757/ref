...
<div class="shadow rounded p-4">
    ...
    @endif
        <div class="container mt-5">
            <form method="post" action="{{ route('update.homepage', ['id' => $homepage->id]) }}">
                @csrf
                @method('patch')
                ...
                @for ($i = 1; $i <= 32; $i++)
                    <div class="form-group">
                        ...
                        <input type="text" class="form-control mb-3" id="h{{ $i }}" name="h{{ $i }}" value="{{ old('h' . $i, $homepage['h' . $i]) }}">
                    </div>
                @endfor
